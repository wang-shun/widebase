package widebase.toolbox.finance.test
...
object Average extends Logger with Loggable {
  ...
  def plotTable(name: String) {
    ...
    plot(
      timeseries(ValueFunction(average), table.time),
      "Color", Color.red)

    hold off

    legend("Table File", "Average")
  ...
  def plotDirTable(name: String) {
    ...
    legend("Directory Table", "Average")
  ...
  def plotPartedDirTable(name: String, from: LocalDate, till: LocalDate) {
    ...
    plot(
      timeseries(ValuePartitionFunction(average), parts.tables("time").Zia),
      "Color", Color.red)

    hold off

    legend("Partitioned Table", "Average")
