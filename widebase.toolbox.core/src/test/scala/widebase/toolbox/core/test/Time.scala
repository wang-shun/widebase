...
object Time extends Logger with Loggable {
  ...
  def plotTable(name: String) {
    ...
    plot(
      timeseries(table.value, table.time),
      "From", 0, "Till", 24)
  ...
  def plotPartedDirTable(name: String, from: LocalDate, till: LocalDate) {
    ...
    plot(
      timeseries(parts.tables("value").dia, parts.tables("time").Zia),
      "From", 0, "Till", 24)

    legend("Partitioned Table")
  ...
  def plotMixedTable(
    ...
    plot(
      timeseries(table.value, table.time),
      "Color", Color.red, "From", 0, "Till", 24)

    hold on

    plot(timeseries(dirTable.value, dirTable.time),
      "Color", Color.green)

    plot(timeseries(parts.tables("value").dia, parts.tables("time").Zia),
      "Color", Color.blue)

    hold off

    legend("Table File", "Directory Table", "Partitioned Table")
